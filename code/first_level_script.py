...
derivative_path = os.path.join(bids_path, 'derivatives', 'fmriprep')
layout = BIDSLayout(bids_path, derivatives=derivative_path)
...
for design_matrix in design_matrices:
    ...
    # ------------------------------------------------------------------
    # Create contrast for Think > No-Think
    # ------------------------------------------------------------------
    # Calculate the number of "Think" and "No-Think" conditions
    num_think = sum(1 for col_name in column_names if col_name.endswith('T'))
    num_nothink = sum(1 for col_name in column_names if col_name.startswith(('negNT', 'neutrNT')))
    ...
    for i, col_name in enumerate(column_names):
        if col_name.endswith('T'):
            contrast_vector[i] = 1 / num_think /nruns # weigh the Think conditions
        elif col_name.startswith(('negNT', 'neutrNT')):
            contrast_vector[i] = -1 / num_nothink/nruns  # weigh the No-Think conditions
    
    contrasts['ThinkNoThink'] = contrast_vector
